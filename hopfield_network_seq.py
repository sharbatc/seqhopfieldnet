...
class HopfieldNetwork:
    ...
    def set_dynamics_sign_sync(self):
        """
        sets the 
        ate dynamics to the synchronous, deterministic g(h) = sign(h) function
        """
        self._update_method = _get_sign_update_function()
    ...
    def store_patterns(self, pattern_list):
        ...
        Edit:
            changed to store a sequence of patterns rather than just a list.
        """
        ...
        for index in np.arange(0,l-1,1):
            ...
            for i in range(self.nrOfNeurons):
                for k in range(self.nrOfNeurons):
                    self.weights[i, k] += next_p_flat[i] * p_flat[k]
...
def _get_prob_sign_update_function():
    """
    for internal use

    Returns:
        A function implementing a synchronous state update using g(h)
    """
    def upd(state_s0, weights,beta):
        h = np.sum(weights * state_s0, axis=1)
        g = 0.5*(1 + np.tanh(beta*h))
        s1 = g
        for index, state in enumerate(g):
            if np.random.rand() < state :
                s1[index] = 1
            else :
                s1[index] = -1
        return s1
